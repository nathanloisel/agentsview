...
// SoftDeleteSessions moves multiple active sessions to user trash in one
// transaction and returns the number changed.
...
// freshness so changes made while it was trashed are parsed.
	...
	err := s.update(ctx, WriteSessionManagement, func(store bun.IDB) error {
		...
		err := store.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
			result, err := tx.NewUpdate().Model((*bunmodel.Session)(nil)).
				...
				Where("id = ?", id).
				Where("deleted_at IS NOT NULL").Exec(ctx)
			if err != nil {
