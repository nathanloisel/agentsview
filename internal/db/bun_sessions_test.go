...
func TestBunStoreListSessionsIncludesSourceOnlyWhenRequested(t *testing.T) {
	raw, err := sql.Open("sqlite3", ":memory:")
	require.NoError(t, err)
	raw.SetMaxOpenConns(1)
	t.Cleanup(func() { require.NoError(t, raw.Close()) })
	...
	_, err = store.NewInsert().Model(&bunmodel.Session{
		...
		FilePath: &sourcePath, SourceArchiveID: "archive",
		SourceDatabaseGeneration: "generation",
	}).Exec(t.Context())
	require.NoError(t, err)

	common := NewBunStore(&sessionContractBackend{store: store})
	withoutSource, err := common.ListSessions(t.Context(), SessionFilter{Limit: 1})
	require.NoError(t, err)
	require.Len(t, withoutSource.Sessions, 1)
	assert.Nil(t, withoutSource.Sessions[0].FilePath)

	withSource, err := common.ListSessions(t.Context(), SessionFilter{
		Limit: 1, IncludeSource: true,
	})
	require.NoError(t, err)
	require.Len(t, withSource.Sessions, 1)
	require.NotNil(t, withSource.Sessions[0].FilePath)
	assert.Equal(t, sourcePath, *withSource.Sessions[0].FilePath)
}
