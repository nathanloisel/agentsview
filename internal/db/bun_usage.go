...
func (s *BunStore) UpsertModelPricing(prices []ModelPricing) error {
	return s.UpsertModelPricingContext(context.Background(), prices)
}

// UpsertModelPricingContext is UpsertModelPricing with caller cancellation.
func (s *BunStore) UpsertModelPricingContext(
...
func bunGenAIEffectivePricingRow(
	...
	var stored bunmodel.GenAIPricing
	err := store.NewSelect().Model(&stored).
		Where("singleton = ?", 1).Limit(1).Scan(ctx)
	...
	if err != nil {
		return export.EffectivePricingRow{}, fmt.Errorf(
			"reading GenAI pricing document: %w", err,
		)
	}
	document, err := pricingpkg.ParseGenAIDocument(
		stored.DataJSON, stored.Version, stored.SourceRef,
	)
	if err != nil {
		return export.EffectivePricingRow{}, fmt.Errorf(
...
func usageProjectionToDailyRow(row bunUsageProjection) dailyUsageScanRow {
	...
	return dailyUsageScanRow{
		...
		usageSource: source, ts: usageProjectionTimestamp(row),
		pricingTS: formatUsageTimestamp(row.UsageTimestamp), model: row.Model,
...
func usageProjectionToFullRow(row bunUsageProjection) usageScanRow {
	...
	return usageScanRow{
		...
		usageSource: daily.usageSource, ts: daily.ts, pricingTS: daily.pricingTS,
		model:     daily.model,
		...
		userMessageCount: row.UserMessageCount, isAutomated: bunBoolInt(row.IsAutomated),
...
func (s *BunStore) loadBunCursorUsageRows(
	...
	for _, row := range rows {
		...
		out = append(out, dailyUsageScanRow{
			...
			pricingTS: formatRequiredUsageTimestamp(row.OccurredAt),
...
func bunBoolInt(value bool) int {
	if value {
		return 1
	}
	return 0
