...
// messageDiffNeedsPinRemap reports whether an in-place update would
...
// the in-place path.
func messageDiffNeedsPinRemap(
	ctx context.Context, store bun.IDB, plan messageDiffPlan,
) (bool, error) {
	if len(plan.unsafePinUpdateIDs) == 0 {
		return false, nil
	}
	var exists int
	err := store.NewRaw(`
