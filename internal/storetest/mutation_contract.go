...
func RunMutationContract(t *testing.T, backend MutationBackend) {
	...
	t.Run(backend.Name+"/soft_delete_preserves_source_state", func(t *testing.T) {
		harness := backend.Open(t, 0)
		...
		for _, id := range []string{
			...
			if id == harness.Rows.SoftSourceMissing ||
				id == harness.Rows.BatchSourceMissing {
	...
	t.Run(backend.Name+"/restore_only_changes_user_trash", func(t *testing.T) {
		harness := backend.Open(t, 0)
		restored, err := harness.Store.RestoreSession(harness.Rows.RestoreSource)
		require.NoError(t, err)
		assert.Zero(t, restored)
		sourceOnly, err := harness.Store.GetSession(t.Context(), harness.Rows.RestoreSource)
		require.NoError(t, err)
		require.NotNil(t, sourceOnly)
		assert.NotNil(t, sourceOnly.SourceMissingAt)
		restored, err = harness.Store.RestoreSession(harness.Rows.RestoreUserTrash)
		require.NoError(t, err)
		...
		assert.NotNil(t, visible.SourceMissingAt)
	...
	t.Run(backend.Name+"/empty_trash_preserves_source_state", func(t *testing.T) {
		harness := backend.Open(t, 0)
		...
		sourceRow, err := harness.Store.GetSession(t.Context(), harness.Rows.RestoreSource)
		require.NoError(t, err)
		require.NotNil(t, sourceRow)
		assert.NotNil(t, sourceRow.SourceMissingAt)
	...
	for index := range rows {
		...
		_, err := tx.ExecContext(ctx, `
			INSERT INTO sessions (
				...
				created_at, deleted_at, source_missing_at, file_path,
			...
			mutationTimestampPointerValue(row.DeletedAt),
			mutationTimestampPointerValue(row.SourceMissingAt),
...
func mutationFixtureRows(
	...
	sourceMissingAt := mutationTimestampPointer("2200-01-01T00:00:00Z")
	...
	rows := []bunmodel.Session{
		...
		mutationSessionRow(fixture.SoftSourceMissing, archiveID, generation, nil, sourceMissingAt),
		mutationSessionRow(fixture.BatchActive, archiveID, generation, nil, nil),
		mutationSessionRow(fixture.BatchSourceMissing, archiveID, generation, nil, sourceMissingAt),
		mutationSessionRow(fixture.BatchAlreadyTrashed, archiveID, generation, userTrash, nil),
		mutationSessionRow(fixture.RestoreUserTrash, archiveID, generation, userTrash, sourceMissingAt),
		mutationSessionRow(fixture.RestoreSource, archiveID, generation, nil, sourceMissingAt),
		...
		mutationSessionRow(fixture.ListSourceMissing, archiveID, generation, nil, sourceMissingAt),
...
func mutationSessionRow(
	...
	sourceMissingAt *bunmodel.Timestamp,
) bunmodel.Session {
	return bunmodel.Session{
		...
		DeletedAt: deletedAt, SourceMissingAt: sourceMissingAt,
